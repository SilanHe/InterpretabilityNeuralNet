...
sst_sentences, sst = sent_util.get_sst_PTB("data/trees", filesplit = "train")
len_sst = len(sst)

start = time.process_time()
for index,tree in enumerate(sst):
	batch = [word.lower() for word in sst_sentences[index]]
	cd,ig,label,_ = sent_util.travelTree_IG_CD(batch, model, inputs, answers, tree, sum) # get ig phrase level scores as well using sum baseline
...
pearson_corr_softmax_cd, _ = pearsonr(list_softmax_cd,list_label)
spearman_corr_softmax_cd, _ = spearmanr(list_softmax_cd,list_label) 

pearson_corr_softmax_ig, _ = pearsonr(list_softmax_ig,list_label)
